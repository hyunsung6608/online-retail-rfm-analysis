...
**핵심 포인트:** 전체 고객의 **22.18%** 에 해당하는 Champions 세그먼트(R≥4, F≥4, M≥4)가
총 매출의 **65.19%** 를 차지했습니다 — 강한 파레토 분포 확인.
...
- **Monetary 로그 변환 (`log1p`) 적용** — 극단적 이상치(예: 1회 구매로 77,183원 지출 고객)가 스코어 분포를 왜곡하는 것을 방지하기 위해 `M_score` 계산 전 로그 변환 수행
...
- **Champions** 세그먼트는 소수이지만 총 매출의 불균형적으로 높은 비중을 차지
...
- **Champions** — 로열티 프로그램, 신제품 얼리 액세스 우선 제공
- **Can't Lose** — 고가 혜택 제공, 즉시 Win-back 캠페인 실행
- **At Risk** — 개인화 메시지, 재방문 유도 쿠폰 발송
- **Loyal** — 업셀·크로스셀 추진, Champions 전환 유도
- **Potential Loyalist** — 멤버십 가입 유도, 반복 구매 혜택 제공
- **New Customer** — 온보딩 이메일, 2차 구매 쿠폰 발송
- **Need Attention** — 시즌 프로모션, 구매 동기 자극
- **Hibernating** — 저비용 리인게이지먼트 또는 마케팅 제외 검토
- **영국 시장 집중** — 지역 확장 기회이자, 영국 수요 약화 시 리스크 요인
